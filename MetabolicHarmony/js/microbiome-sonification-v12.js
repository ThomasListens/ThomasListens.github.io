/**
 * MicrobiomeSonification v12 - FOCUS SHAPING + MS SPOTLIGHT + KEY FINDINGS
 * 
 * Changes from v11:
 * - FOCUS TONE: Fletcher-Munson compensation (boost lows, tame highs)
 * - FOCUS TONE: Gentler ducking with slower attack
 * - MS SPOTLIGHT: Lightweight layer guarantees MS pathways are heard
 * - KEY FINDINGS ENGINE: Chord with phase-offset ripple modulation
 * 
 * Architecture:
 ...
 * - MSSpotlightLayer     (NEW: guarantees MS pathway audibility during transition)
 * - FocusEngine          (single pathway focus tone - now with EQ shaping)
 * - KeyFindingsEngine    (NEW: rippling chord for pathway groups)
...
class MSSpotlightLayer {
    constructor() {
        this.config = {
            ...
            baseVolume: 0.0015,      // Subtle - supplement not replace
...
class FocusEngine {
    constructor() {
        this.config = {
            targetVolume: 0.045,     // Slightly reduced base
            attackTime: 0.15,
            releaseTime: 0.4,
            harmonics: [
                { ratio: 1, amp: 1.0 },
...
class KeyFindingsEngine {
    constructor() {
        this.config = {
            ...
            maxVoices: 12,           // Limit for very large findings
...
class MicrobiomeSonificationProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.msSpotlight = new MSSpotlightLayer();  // NEW
        this.focus = new FocusEngine();
        this.keyFindings = new KeyFindingsEngine();  // NEW
    ...
    process(inputs, outputs, parameters) {
        ...
        this.keyFindings.process(this.ctx, dryL, dryR, reverbSendL, reverbSendR);  // NEW
